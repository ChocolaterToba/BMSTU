...
void Planet::update(char* new_name, int new_diameter, bool new_life, int new_satellites) { // So that Planet stays in sight area.
	set_name(new_name);
...
ofstream& operator<< (ofstream& out, Planet& planet) {
	out << planet.name << " " // Ask about why, if you put a method there, it breaks. And also ask why there was a problem with const Planet& planet.
		<< planet.diameter << " "
		<< planet.life << " "
