...
int main() {
	...
	Planet solar_system[Size]; // Ask about why if you make planets[n] a new Planet object it calles destructor after.
	int n_planet = 0;
	int ind;
	while (true) {
		switch (menu())
...
void delete_planet(Planet* planets, int index, int& n_planet) {
	...
	planets[n_planet - 1].update("\0", 1, 0, 0);
	n_planet--;
}
